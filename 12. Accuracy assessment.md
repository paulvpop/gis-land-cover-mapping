...
```
str(sample_gpkg)
# Reading layer `sample' from data source `D:\Paul_Pop_RS_GIS_files\Top_Models_Organized\sample.gpkg' using driver `GPKG'
...
```
setwd("D:/Paul_Pop_RS_GIS_files/LULC-SIANG/Reorganized_Rasters_Categorical")
```
...
for (i in 1:nrow(mod_det)) {
  ...
  all_class_levels <- unique(c(true_classes, predicted_classes))
  
  # Convert to factor with all levels to ensure they appear in the table
  conf_matrix <- table(
