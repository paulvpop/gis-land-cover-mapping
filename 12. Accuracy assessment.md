...
As [this paper]([url](https://samv.elearning.unipd.it/pluginfile.php/175898/mod_resource/content/0/articolo_oloffson.pdf)) (Good practices for estimating area and assessing accuracy of land change)
...
6) A study area shapefile in the input directory with the name study_area.shp within the subfolder study_area.
...
7) Load the CSV file with model information (replace the file name with your file name).
8) Under the "# Pre-compute class mapping (constant)", change the abbreviation of classes to that of yours.
...
```
output_dir <- "pixel_count" #Rename as pixel_count_Windows, pixel_count_linux etc
```
Depending on whether you have the input processed files of various models
...
```
Main processing loop (Finished in ~4 minutes with 22 cores (and 32 GB RAM), and 
    ...
    for (class in classes) {
      if (class %in% names(result$counts)) {
        ...
        if (length(vals) > 0) {
          ...
          # lower bound = Q[1] - 1.5*(Q[2]-Q[1]), and upper bound = Q[2] + 1.5*(Q[2]-Q[1])
          ...
          if (length(vals_clean) > 0) {
            avg_data <- rbind(avg_data, data.frame(
              ...
              #Standard deviation
              sd = sd(vals_clean),
              #Number of models used for the calculation that satisfy the criteria
              n = length(vals_clean)
            ))
          }
        }
