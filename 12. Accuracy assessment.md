...
for (i in 1:nrow(mod_det)) {
  ...
  # Instead of kappa, use quantity disagreement and allocation disagreement instead.
  classes <- colnames(conf_matrix)[colnames(conf_matrix) != "Sum"]  # Exclude 'Sum'
  ...
  if (length(classes) == length(diag_values) &&
    ...
    # Per-class precision, recall, and F-1
    precision = diag_values / (full_rowsums + nc_ordered)
    recall = diag_values / (full_colsums + nc_ordered)  
    f1 = 2 * ((precision * recall) / (precision + recall))
