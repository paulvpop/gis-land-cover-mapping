...
- [Unoptimised and detailed workflow:](#unoptimised-and-detailed-workflow)
 ...
 - [Optimised updated workflow:](#optimised-updated-workflow)
   ...
   - [Calculate the per strata sample size](#calculate-the-per-strata-sample-size)
     + [Using FORCE](using-force) <br>
      [Create inputs for the per strata sample size calculation in FORCE](#create-inputs-for-the-per-strata-sample-size-calculation-in-force) <br>
...
# Unoptimised and detailed workflow
...
in this workflow based on published literature)

#### Using R

As the study area is composed of around 10,674,943.042 acres (4,32,00,000,000 m2),
and the number of classes is more than 11 (n = 13), as per the book  "Assessing 
the Accuracy of Remotely Sensed Data", the sample size per class should be between
75 to 100. So, in the following script, the minimum sample size per class will be 
kept as 75.
