...
The following workflow can be used for accuracy assessment of those land cover maps produced in the last section (after post-processing).  
...
- *[Unoptimised, detailed, and retired workflow:](#unoptimised-detailed-and-retired-workflow)*
 ...
 - [Optimised updated workflow:](#optimised-updated-workflow)
   ...
   - [Create stratified random samples](#create-stratified-random-samples)
      + [Create input for the force-stratified-sample module of FORCE](#create-input-for-the-force-stratified-sample-module-of-force)
      + [Run the force-stratified-sample module in FORCE](#run-the-force-stratified-sample-module-in-force)
   - [Create input for QGIS and Google Earth](#create-input-for-qgis-and-google-earth)
   - [Relabel the stratified random sample points](#relabel-the-stratified-random-sample-points)
   - [Collect additional validation points if required](#collect-additional-validation-points-if-required)
   - [Accuracy assessment](#accuracy-assessment)
   - [Statistics aggregation of accuracy assessment](#statistics-aggregation-of-accuracy-assessment)
...
The standalone R script for this section can be found [here](https://github.com/paulvpop/gis-land-cover-mapping/blob/main/R_script_for_pixel_count_and_proportions.R).
...
The standalone R script for this section can be found [here].(https://github.com/paulvpop/gis-land-cover-mapping/blob/main/R_script_for_pixel_statistics_aggregation.R).
...
The standalone R script for this section can be found [here](https://github.com/paulvpop/gis-land-cover-mapping/blob/main/R_script_for_calculating_the_per_strata_sample_size).
...
# 1408
```
1408 validation points need to be taken which is not as horrific a task as the horrors of the 2007 movie of the same name.
...
```
Since we are going to assess the accuracy of both Sentinel and Landsat model outputs,
we should take the model with the larger number of validation data i.e. 1408 (from Landsat).

## Create stratified random samples

#### Create input for the force-stratified-sample module of FORCE

The standalone R script for this section can be found [here](https://github.com/paulvpop/gis-land-cover-mapping/blob/main/R_script_for_creating_input_for_the_force-stratified-sample_module_of_FORCE.R).
...
The standalone R script for this section can be found [here](https://github.com/paulvpop/gis-land-cover-mapping/blob/main/R_script_for_creating_input_for_QGIS_and_Google_Earth.R).
...
used for the next step. Save this file as sample_recoded.kml. 

## Collect additional validation points if required

Once the re-labelling has been done and the sample_recoded.kml has been downloaded,
note that we will not be left with the exact proportion of samples we estimated in step - 
Calculate the per strata sample size (using R). Those 1408 samples for different classes
can be jumbled up for several classes even if the classified map used as the input for
the generation/collection of validation data from the stratified map has good accuracy.
If classes have a lower than threshold number of validation data (75 in my case), then
those classes need additional validation data so that it reaches 75.

First check which classes need additional validation points by doing the following. Under the 
[Sampling section Step A.2](https://github.com/paulvpop/gis-land-cover-mapping/blob/main/07.%20Sampling.md#step-a2-create-the-txt-file-containing-the-training-data-coordinates-and-class-numbers), Follow
the steps A.2.i.1, A.2.i.2, and A.2.i.3 till `table(samples$Name)`. But instead of the 
training data file in the Sampling section, use the validation data i.e. sample_recoded.kml.
This will give you the number of validation points for each classes. 

Now open the sample_recoded.kml in Google Earth (Pro) or QGIS (if not already open)
and collect more points for which there are lesser-than-threshold numbers. Then save the file
as final_sample_recoded.kml.

## Accuracy assessment

The standalone R script for this section can be found [here](https://github.com/paulvpop/gis-land-cover-mapping/blob/main/R_script_for_accuracy_assessment.R).
...
for (i in 1:nrow(mod_det)) {
  ...
  # _processed suffix)
  tif_files <- list.files(getwd(), pattern = "_processed\\.tif$", full.names = TRUE) # Unchanged workflow: change _processed to _unchanged
...
Output:
```
[1] "This will appear in the console and be saved to output.txt"

==================================================
Processing model: dir_ML_sent_RFC_13
==================================================

Model predicts/outputs these classes: AGR, AJ, BA, BU, CC, MGR, OF, SH, SN, SS, TF, WA, WRC 
No recoding of class applied
...
Final results after buffer enhancement:
   ...
   0    1 
   9 1592 
Classes in conf_matrix: AGR, AJ, BA, BU, CC, MGR, OF, SH, SN, SS, TF, WA, WRC 
Classes in nc:          AGR, AJ, BA, BU, CC, MGR, OF, SH, SN, SS, TF, WA, WRC 

Overall Accuracy: 91.44 

✓ Successfully completed processing for model: dir_ML_sent_RFC_13 

==================================================
Processing model: dir_ML_sent_RFC_14
==================================================
...
✓ Successfully completed processing for model: dir_ML_sent_RFC_14 
.
.
.
==================================================
Processing model: dir_ml_RFC_41
==================================================

Model predicts/outputs these classes: AGR, AJ, BU, CC, MGR, OF, SN, SS, TF, WA, WRC 
Recoded BA and SH as TF
...
Final results after buffer enhancement:
   ...
   0    1 
  14 1587 
Classes in conf_matrix: AGR, AJ, BU, CC, MGR, OF, SN, SS, TF, WA, WRC 
Classes in nc:          AGR, AJ, BU, CC, MGR, OF, SN, SS, TF, WA, WRC 

Overall Accuracy: 87.26 

✓ Successfully completed processing for model: dir_ml_RFC_41 

==================================================
Processing model: dir_ml_RFC_42
==================================================
...
✓ Successfully completed processing for model: dir_ml_RFC_42 

==================================================
Processing model: dir_ML_pbc_sent_RFC_1
==================================================

SKIPPING MODEL: dir_ML_pbc_sent_RFC_1 - Directory not accessible

==================================================
Processing model: dir_ML_pbc_sent_RFC_2
==================================================

SKIPPING MODEL: dir_ML_pbc_sent_RFC_2 - Directory not accessible
.
.
.

==================================================
Loop completed! Saved the accuracy metrics of 261 out of 354 models (some models may go unprocessed if there is insufficient data)
==================================================
```
To give a sense of the time it will take, it took around 21 minutes in the Windows desktop to process 261 models (mix of Landsat and Sentinel).
It took around 10 minutes in the Linux desktop to process 93 models (mix of Landsat and Sentinel). Refer to the specifications of
these systems in the Introduction of this GitHub repository.

## Statistics aggregation of accuracy assessment

The standalone R script for this section can be found [here](https://github.com/paulvpop/gis-land-cover-mapping/blob/main/R_script_for_statistics_aggregation_of_accuracy_assessment.R).
...
Output:
```
Number of valid directories in the system: 261 
```
...
Output:
```
Created: class_AGR_Landsat_metrics.csv with 167 models
Created: class_AJ_Landsat_metrics.csv with 167 models
Created: class_BA_Landsat_metrics.csv with 82 models
Created: class_BU_Landsat_metrics.csv with 167 models
Created: class_CC_Landsat_metrics.csv with 167 models
Created: class_MGR_Landsat_metrics.csv with 167 models
Created: class_Mean_Overall_Landsat_metrics.csv with 167 models
Created: class_OF_Landsat_metrics.csv with 167 models
Created: class_SH_Landsat_metrics.csv with 87 models
Created: class_SN_Landsat_metrics.csv with 167 models
Created: class_SS_Landsat_metrics.csv with 167 models
Created: class_TF_Landsat_metrics.csv with 167 models
Created: class_WA_Landsat_metrics.csv with 167 models
Created: class_WRC_Landsat_metrics.csv with 167 models
Created: class_AGR_Sentinel_metrics.csv with 60 models
Created: class_AJ_Sentinel_metrics.csv with 60 models
Created: class_BA_Sentinel_metrics.csv with 30 models
Created: class_BU_Sentinel_metrics.csv with 60 models
Created: class_CC_Sentinel_metrics.csv with 60 models
Created: class_MGR_Sentinel_metrics.csv with 60 models
Created: class_Mean_Overall_Sentinel_metrics.csv with 60 models
Created: class_OF_Sentinel_metrics.csv with 60 models
Created: class_SH_Sentinel_metrics.csv with 30 models
Created: class_SN_Sentinel_metrics.csv with 60 models
Created: class_SS_Sentinel_metrics.csv with 60 models
Created: class_TF_Sentinel_metrics.csv with 60 models
Created: class_WA_Sentinel_metrics.csv with 60 models
Created: class_WRC_Sentinel_metrics.csv with 60 models
```
...
Output:
```
Created: class_AGR_Landsat_metrics_final.csv with 195 models
Created: class_AJ_Landsat_metrics_final.csv with 195 models
Created: class_BA_Landsat_metrics_final.csv with 100 models
Created: class_BU_Landsat_metrics_final.csv with 195 models
Created: class_CC_Landsat_metrics_final.csv with 195 models
Created: class_MGR_Landsat_metrics_final.csv with 195 models
Created: class_Mean_Overall_Landsat_metrics_final.csv with 195 models
Created: class_OF_Landsat_metrics_final.csv with 195 models
Created: class_SH_Landsat_metrics_final.csv with 105 models
Created: class_SN_Landsat_metrics_final.csv with 195 models
Created: class_SS_Landsat_metrics_final.csv with 195 models
Created: class_TF_Landsat_metrics_final.csv with 195 models
Created: class_WA_Landsat_metrics_final.csv with 195 models
Created: class_WRC_Landsat_metrics_final.csv with 195 models
Created: class_AGR_Sentinel_metrics_final.csv with 120 models
Created: class_AJ_Sentinel_metrics_final.csv with 120 models
Created: class_BA_Sentinel_metrics_final.csv with 60 models
Created: class_BU_Sentinel_metrics_final.csv with 120 models
Created: class_CC_Sentinel_metrics_final.csv with 120 models
Created: class_MGR_Sentinel_metrics_final.csv with 120 models
Created: class_Mean_Overall_Sentinel_metrics_final.csv with 120 models
Created: class_OF_Sentinel_metrics_final.csv with 120 models
Created: class_SH_Sentinel_metrics_final.csv with 60 models
Created: class_SN_Sentinel_metrics_final.csv with 120 models
Created: class_SS_Sentinel_metrics_final.csv with 120 models
Created: class_TF_Sentinel_metrics_final.csv with 120 models
Created: class_WA_Sentinel_metrics_final.csv with 120 models
Created: class_WRC_Sentinel_metrics_final.csv with 120 models
```
