...
- [Step 5: Run the program](#step-5-run-the-program)
- [Step 6: Convert single-model single-band rasters or multi-models-multi-band-rasters to single-model-multi-band-rasters](#step-6-convert-single-model-single-band-rasters-or-multi-models-multi-band-rasters-to-single-model-multi-band-rasters)
...
ONLY NEEDED IF YOU'RE RUNNING MULTIPLE MODELS AT ONCE
...
## Step 6: Convert single-model single-band rasters or multi-models-multi-band-rasters to single-model-multi-band-rasters
...
For the purpose of the batch processing, you need to have a csv file containing the following details (even if it is just one model, the details need to be entered in the csv file).
...
The following script can be used for batch restructuring of rasters for a large number of models. It can either be single-model single-band rasters (those single-bands containing every class of that model as pixels) or multi-model multi-band rasters (each band containing one model output, where each band contains every class of that model as pixels) or a mix of both.

This is available as standalone R script [here](R_script_for_conversion_of_single-model_single-band_rasters_or_multi-models_multi-band_rasters_to_single-model_multi-band_rasters.R).

Note that in the  # Generate class values section of the script, if you haven't provided the max_classes (directly in the script) and missing_classes in the csv file, then it will attempt to derive that data by checking 50,000 pixels of the rasters. But this is inefficient and incomplete (and not thoroughly stress-tested). So, you can either increase 50,000 to a much larger number for accuracy or avoid the processing time and power for this, and manually specify the values. 
...
4) The parse band function assumes that your model band names are of the general structure MODEL_CLASS_0000X_Y  where X could be 1 and Y could be EQUALIZED; 0001 refers to the number for EQUALIZED and EQUALIZED is the model containing equalised samples from each class. You can modify the function to account for your model band names.
5) You need to change the value for the max_classes (depending on your total number of classes) under the line # Global constant for number of classes in all the models AND the second line after # Processing function for a single model
...
# A number of functions need to be created for the steps like finding the location of the models specified in the csv file,
...
This is available as standalone R script [here](R_script_for_extraction_of_individual_model_bands_from_multi_model_multi_band_rasters.R).
