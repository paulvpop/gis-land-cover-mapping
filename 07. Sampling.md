...
#Recode the names:
#WARNING: DO NOT CHANGE THE NUMBERS FOR EACH CLASS FOR DIFFERENT MODELS. FOR EXAMPLE, IF ONE MODEL IS HAVING 11 CLASSES (MISSING BA FOR EXAMPLE) INSTEAD OF 12,
#DO NOT CHANGE THE CLASS NAME OF THE LAST BAND TO 11 INSTEAD OF 12. SO, SS SHOULD ALWAYS BE 12 AND SHOULD NOT 11 OR SOME OTHER NUMBER FOR A DIFFERENT MODEL.
samples <- samples %>%
    select ("X","Y","Name") %>%
...
**ONLY NEEDED FOR THE RETIRED NON_RECOMMENDED WORKFLOW**

Carrying out this step results in individual predictions for each classes, but this results in a lot of gaps after the integration stage (70 or more %). This alternate workflow has been retired and is not recommended unless one intends to study the effects of doing this, or finding exceptionally good gap-filling techniques before or after the integration stage.
