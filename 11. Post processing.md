...
- [Workflow 2 - Minimalist and optimised](#workflow-2---minimalist-and-optimised)
   - [Creation of rasters with probabilistic landscape for each class](#creation-of-rasters-with-probabilistic-landscape-for-each-class)
   - [Generation of random points for validation](#generation-of-random-points-for-validation)
   - [Bulk post-processing of model outputs](#bulk-post-processing-of-model-outputs)
   - [Bulk merging of unchanged model outputs](#bulk-merging-of-unchanged-model-outputs)
...
ctrl+Shift+H OR 
```
setwd("D:/GIS/LULC-SIANG/classified_imagery")
```
Check the current working directory (optional)
```
getwd()
```
Install the packages 'raster' and 'sp' if not already installed. 
Note that you may have to install or update some dependencies if the installation 
is not working. Check the errors/warnings. 
```
...
head(data)
```
Change working directory to where the DEM to be used is located (or else, move the DEM to the 
current working directory): 
```
setwd("D:/GIS/LULC-SIANG/DEM")
```
...
class which can then be independently checked in Google Earth (or QGIS with relevant 
...
```
## Bulk post-processing of model outputs
